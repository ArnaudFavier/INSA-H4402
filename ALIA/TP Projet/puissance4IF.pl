...
play(Player):-  write('New turn for:'), writeln(Player),
            ...
            write('Joueur '), write(Player), write(' entrez un numéro de colonne : '),
            read(Move), 
...
% OK !
printLine(N):- board(B),  nth1(X, B, Val), nth1(N, Val, V), write(V), write('|'), X > 6, writeln('').
